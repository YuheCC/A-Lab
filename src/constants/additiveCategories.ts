...
export const ADDITIVE_OPTIONS_BY_CATEGORY: Record<AdditiveCategoryType, AdditiveOptionConfig[]> = {
  mechanistic: [
...
export const isValidAdditiveSubtype = (
  ...
  subtype: string,
): boolean => ADDITIVE_OPTIONS_BY_CATEGORY[category].some((option) => option.value === subtype);

export const getDefaultSubtypeForCategory = (category: AdditiveCategoryType): string =>
  DEFAULT_ADDITIVE_SUBTYPE[category];
