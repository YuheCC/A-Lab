export default {
    ...
    introduction: {
        ...
        example: {
            title: "示例",
            paragraph1: "在一个示例中，基于模型的预测通过捕捉随时间演变的SOH退化趋势，估计电池寿命终止约在1308个循环。与假设恒定衰减率不同，预测投射出非线性老化行为，产生与测量的SOH演变一致的轨迹。在这种情况下，模型使用垂直红色虚线指示的循环之前的数据（约143个循环），并排除异常循环以建立稳定的退化趋势进行预测。实际寿命终止观察在1396个循环（不包括容量检查循环），对应约6.3%的预测误差。",
