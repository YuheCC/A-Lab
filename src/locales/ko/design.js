export default {
    ...
    electrode: {
        ...
        introduction: {
            ...
            resultPrediction: {
                title: "1. 결과 예측",
                description1: "결과 예측은 정의된 셀 설계 내에서 전극 재료 선택 및 배합 매개변수를 기반으로 에너지 밀도, 속도 성능 및 사이클 수명(향후 개발)을 포함한 셀 수준 성능에 대한 데이터 기반 예측을 제공합니다.",
                description2: "SES의 독점 실험 데이터베이스에서 학습된 AI 모델로 구동되는 이 모듈은 물리적 프로토타이핑 없이도 특정 설계 구성을 신속하고 확장 가능하게 평가할 수 있게 합니다. 엔지니어가 주어진 셀 설계 및 배합의 예상 성능 결과를 평가하여 후보 설계의 효율적인 스크리닝 및 비교를 지원합니다.",
                exampleTitle: "예",
                example: "특정 양극 및 음극 배합과 결합된 주어진 셀 설계의 사이클 수명 및 속도 성능을 예측합니다."
            },
            inverseDesign: {
                title: "2. 역설계",
                description1: "역설계는 사용자 정의 성능 목표를 충족하는 전극 재료 및 배합 매개변수의 실현 가능한 조합을 자동으로 생성합니다. 설계 공간을 수동으로 반복하는 대신, 이 모듈은 현실적인 물리적 및 제조 제약 내에서 검색하여 유효한 설계 후보를 식별합니다. 이 접근 방식은 에너지 밀도, 속도 성능 및 수명 간의 절충안을 밝혀 다목적 의사 결정을 지원하면서 비현실적이거나 불안정한 솔루션을 피합니다.",
                description2: "역설계는 예측 및 트렌드 분석의 통찰력을 기반으로 구축되어, 고성능이면서 제조 가능한 설계 옵션의 목표 기반 탐색을 가능하게 합니다.",
                exampleTitle: "예",
                example: "높은 속도 성능과 목표 사이클 수명을 유지하면서 1000Wh/L를 달성하는 전극 배합을 설계합니다。"
            }
        },
        predict: {
            title: "성능 예측",
