export default {
    ...
    introduction: {
        ...
        example: {
            title: "例",
            paragraph1: "ある例では、モデルベースの予測が、時間の経過に伴うSOH劣化トレンドを捉えることで、バッテリー寿命終了を約1308サイクルと推定しました。一定の劣化率を仮定するのではなく、予測は非線形な劣化挙動を投影し、測定されたSOH変化と一致する軌跡を生成しました。この場合、モデルは垂直の赤い破線で示されたサイクルまでのデータ（約130サイクル）を使用し、異常なサイクルを除外して安定した劣化トレンドを確立しました。実際の寿命終了は1396サイクル（容量チェックサイクルを除く）で観測され、約6.3%の予測誤差に相当します。",
