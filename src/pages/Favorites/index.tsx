...
const FavoritesGrid = () => {
  ...
  const updateSpiderChart = () => {
    ...
    const categories = [
      t('favorites.chartProperties.homo'),
      t('favorites.chartProperties.lumo'),
      t('favorites.chartProperties.meltingPoint'),
      t('favorites.chartProperties.boilingPoint'),
      t('favorites.chartProperties.molecularWeight')
    ];
  ...
  const updateESPChart = () => {
    ...
    const getEllipsePoints = (center: [number, number], width: number, height: number, angleDeg: number, nPoints = 100) => {
    ...
    const customEllipses = [
      {
        center: [-1.5, 1.6] as [number, number],
        width: 1.5,
        height: 0.55,
        angle: 65,
        color: 'green',
      ...
      {
        center: [-0.75, 0.65] as [number, number],
        width: 0.6,
        height: 0.6,
        angle: 0,
        color: 'red',
      ...
      {
        center: [-0.55, 1.6] as [number, number],
        width: 0.65,
        height: 1.3,
        angle: 0,
        color: 'blue',
    ...
    const data = selectedMolecules.map(molecule => ({
      ...
      SOLUBILITY: molecule.solubility || t('favorites.chartLabels.selectedMolecules'), // Assuming solubility field exists
    ...
    const groupedData: Record<string, typeof validData> = {};
    validData.forEach(d => {
    ...
    customEllipses.forEach(ellipse => {
      ...
      const colorToRgba = (color: string, alpha = 0.2) => {
    ...
    const layout = {
      title: {
        text: t('favorites.chartTitles.esp')
      },
      xaxis: {
        title: {
    ...
    if (espChartRef.current) {
      (espChartRef.current as any).on('plotly_click', handlePointClick);
    }
  };
  ...
  const updateMOChart = () => {
    ...
    const selectedData = selectedMolecules.length > 0 ? selectedMolecules.map(molecule => ({
      HOMO_EV: molecule.homo_ev,
      LUMO_EV: molecule.lumo_ev,
      SOLUBILITY: molecule.solubility || t('favorites.chartLabels.selectedMolecules'),
    ...
    const addJitter = (data: any[], jitterAmount = 0.02) => {
    ...
    allData.forEach(d => {
      if (d.isSelected) {
        ...
        const selectedKey = t('favorites.chartLabels.selectedMolecules');
        if (!groupedData[selectedKey]) {
          groupedData[selectedKey] = [];
        }
        groupedData[selectedKey].push(d);
      } else {
        // Keep reference data grouped by solubility
        const key = `${d.SOLUBILITY} (${t('favorites.chartLabels.reference')})`;
        if (!groupedData[key]) {
          groupedData[key] = [];
        }
    ...
    legendOrder.forEach(solubilityType => {
      ...
      if (groupedData[refKey]) {
        ...
        traces.push({
          ...
          text: typeData.map(d => d.ABBREVIATION),
          textposition: 'top center',
          name: `${solubilityType} (${t('favorites.chartLabels.reference').toLowerCase()})`,
          hovertemplate: 
    ...
    const selectedKey = t('favorites.chartLabels.selectedMolecules');
    if (groupedData[selectedKey]) {
      const selectedData = groupedData[selectedKey];
    ...
    const layout = {
      title: {
        text: t('favorites.chartTitles.mo')
      },
      xaxis: {
        title: {
    ...
    if (moChartRef.current) {
      (moChartRef.current as any).on('plotly_click', handlePointClick);
    }
  };
