...
export interface AnionsFilterRef {
    resetFilters: () => void;
    getFilteredData: () => any[];
}

interface AnionsFilterProps {
    onDataFiltered: (data: any[]) => void;
}

// Filter labels for anions
export const anionsFilterLabels: { [key: string]: string } = {
...
export const anionsFunctionGroupOptions: FunctionalGroupOption[] = [
    { label: "Carboxylate", value: "[C](=O)[O-]" },
    { label: "Sulfonate", value: "[S](=O)(=O)[O-]" },
    { label: "Phosphate", value: "[P](=O)([O-])([OH,O-])[OH,O-]" },
    { label: "Nitrate", value: "[N+](=O)([O-])[O-]" },
    { label: "Hydroxide", value: "[OH-]" },
    { label: "Halide", value: "[F-,Cl-,Br-,I-]" },
    { label: "Acetate", value: "[CH3]C(=O)[O-]" },
    { label: "Bicarbonate", value: "[O-]C(=O)O" },
    { label: "Perchlorate", value: "[Cl+3]([O-])([O-])([O-])[O-]" },
    { label: "Tetrafluoroborate", value: "[B-]([F])([F])([F])[F]" },
    // 可以根据实际的 anions 数据添加更多功能组
];

const AnionsFilter = forwardRef<AnionsFilterRef, AnionsFilterProps>(({ onDataFiltered }, ref) => {
    const { t } = useTranslation();
    const { userPermissions, isAuthenticated } = useAuthStore();
    const { data } = useAnionsPlotDataStore();
    ...
    const getFilterLabel = (property: string): string => {
        return t(`explorer.filterLabels.${property}`, anionsFilterLabels[property]);
    };
    ...
    const resetFilters = () => {
        setFilterRanges(prev => {
            ...
            for (const [key, range] of Object.entries(prev)) {
                if (anionsFilterLabels[key as keyof typeof anionsFilterLabels]) {
                    newRanges[key] = {
                        ...range,
...
AnionsFilter.displayName = 'AnionsFilter';

export default AnionsFilter;